...
\begin{lstlisting}
$ opam install -y ocaml-lsp-server ocamlformat ounit2 utop
\end{lstlisting}

\subsection{Setting up text editor}\label{TextEditorSetup}
...
\subsection{Install Docker Desktop}\label{InstallDocker}
...
\subsection{Build the container from \texttt{cse305\_dev\_env} repository}\label{BuildContainer}
...
If you would like to use Emacs as your text editor, read Section~\ref{EmacsSetup},
otherwise, if you would like to use VS Code, read Section~\ref{VSCodeSetup}.

\subsubsection{Using Emacs in terminal}\label{EmacsSetup}
...
at \texttt{/home/cse305/workspace}.
...
\subsubsection{Using VS Code with plugins}\label{VSCodeSetup}
\begin{tcolorbox}[
  colback=yellow!10,
  colframe=red!70!black,
  title=⚠ Read and understand the following notice regarding Academic integrity
]
The use of AI (including AI autocompletion and other interfaces) is not permitted in this course. Given the changes to VS Code which made AI autocompletion on by default recently, it is your responsibility to ensure that you have disabled any AI autocompletion features such as Copilot in VS Code while working on course assignments. If you want to complain about you are not sure whether your VS Code is using AI autocompletion, please use Emacs (in Section~\ref{EmacsSetup}) or \href{https://ocamlverse.net/content/editor_setup.html#vim-or-neovim}{Vim}, which will not have this problem, and we are happy to help you set it up.
\end{tcolorbox}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{vscode_start_container.png}
\end{figure}

After the container is started, right-click on it again and select ``Attach Visual Studio Code'', a new window will open and attach to the container.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{vscode_attach_editor.png}
\end{figure}

The new window will open and attach to the container, but it may not open the workspace folder automatically.
To open the workspace folder, click on ``Open Folder'' or go to \texttt{File > Open Folder$\ldots$}, then choose \texttt{/home/cse305/workspace} and click ``OK''. Workspace folder with pre-existing ``\texttt{test.ml}'' should open.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{vscode_open_workspace.png}
  \includegraphics[width=0.5\textwidth]{vscode_click_ok.png}
\end{figure}
...
\subsection{I ran \texttt{which ocaml} and it said \texttt{ocaml: command not found!}}

If closing and reopening the terminal or restarting machine doesn't help,
...
\subsection{VSCode syntax highlighting and autocomplete not working}

You need to install \texttt{ocaml-lsp-server} and \texttt{ocamlformat} by running:
\begin{lstlisting}
opam install -y ocaml-lsp-server ocamlformat
\end{lstlisting}

\subsection{I tried to build the Docker image, but it failed with an error.}

Make sure you read the error message carefully.
It often provides clues about what went wrong.
Please let us know if you are unable to resolve the issue.

\subsection{Received an error ``\texttt{Cannot connect to the Docker daemon at docker.sock. Is the docker daemon running?}''/``\texttt{Docker error the docker daemon is not running}''}

Make sure Docker Desktop is running before you try to run any Docker commands.

\subsection{Docker Desktop shows an error ``\texttt{WSL needs updating: Your version of Windows Subsystem for Linux is too old}''}
Go to \href{https://github.com/microsoft/WSL/releases}{WSL GitHub Releases}, download the \texttt{.msi} installer from the latest stable release under the Assets section, if you are using amd64 architecture, download \texttt{wsl.\#.\#.\#.x64.msi}, if you are using arm64 architecture, download \texttt{wsl.\#.\#.\#.arm64.msi}. Run the installer and follow the setup instructions, and restart your machine when it's done.
